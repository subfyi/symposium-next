...
import { useLogin, useLoginGuard } from '@/providers/LoginProvider'
...
function UserLogin() {
  ...
  const login = useCallback(function(e: Event) {
    e.preventDefault()
    _login(1)
  }, [_login])
  ...
  if (!!user) {
    return <>
      <li className='action__text'>
        <a href='https://portal.iseser.com/'>Hello, {user.name} {user.surname}</a>
      </li>
      <li className='action__text'>
